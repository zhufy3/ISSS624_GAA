...
By examine the gap statistic graph, the 7-cluster gives the largest gap statistic.
...
```{r}
nga_sf_cluster <- cbind(nga_wp, as.matrix(groups)) %>%
...
First, we need to convert nga_wpt into SpatialPolygonsDataFrame called *nga_sp*. This is because SKATER function only support **sp** objects such as SpatialPolygonDataFrame.
...
Next, `nbcosts()` of **spdep** package is used to compute the cost of each edge. It is the distance between it nodes. This function compute this distance using a data.frame with observations vector in each node. But this function requires no disconnected nodes. Therefore we first extract the region having no neighbors. The code chunk below is used to compute the cost of each edge.
