...
Other methods of agnes why are not built in the function?

Use transform function to transform the crs

Other methods to determining optimal clusters
...
-   Total number of water points at different population densities

-   Total number of water points at different local population densities
...
### 7 Correlation Hierarchy Analysis

#### 7.1 Extracting clustering analysis
...
##### 7.1.2 Visualising standardised variable
...
Before that, we create a tidier data frame named *nga_5,* which only contains 5 variables, from *nga_wp .*
...
Now we use spatially constrained clustering to take spatial attribute into consideration.
...
Take a look at the file *nga_wp*, this region number 86 is named ***Bakassi***, with other attributes all 0.![](images/tempsnip-02.png)
...
We can see from the map above, a little region in red border is at the south of the country having no neighbors.

#### 8.3 Computing minimum spanning tree

##### 8.3.1 Calculating edge costs

Next, `nbcosts()` of **spdep** package is used to compute the cost of each edge. It is the distance between its nodes. This function compute this distance using a data.frame with observations vector in each node. But this function requires no disconnected nodes. Therefore we first extract the region **86** having no neighbors. The code chunk below is used to compute the cost of each edge.

```{r}
nga.nb0<-nga.nb[-86]
class(nga.nb0)<-"nb"
rownames(wpt_clu0)<-NULL
lcosts <- nbcosts(nga.nb0,wpt_clu)
lcosts
...
```{r}
nga.w <- nb2listw(neighbours=nga.nb0, 
                   glist=lcosts, 
                   style="B",
                  zero.policy = TRUE)
...
```{r}
schc<-qtm(nga_sf_Gcluster, "CLUSTER", title="Chrolopeth Map Based on Spatially Constrained Hierarchical Clustering")
hc<-qtm(nga_sf_cluster, "CLUSTER", title ="Chrolopeth Map Based on Hierarchical Clustering")
tmap_arrange(schc,hc,
             ncol=2,
             nrow=1)
```
