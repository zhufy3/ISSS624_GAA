...
Next, we need to assign weights to each neighboring polygon based both on ***Queen*** and ***Rook***. In our case, each neighboring polygon will be assigned equal weight
(style=\"W\").
...
```{r}
knn6 <- knn2nb(knearneigh(coords, k=8))
knn6
```
...
From the summary of the test result, we can see that the **p-value** is less than **0.05**, which means we have evident reasons to reject the Null Hypothesis that the percentage of Non-Functional water point is randomly distributed across the country. The **Geary C statistic = 0.4489704923 \< 1.**
...
From the summary of the test result, we can see that the **p-value** is **0.001,** still less than **0.05**, which means we have evident reasons to reject the Null Hypothesis that the percentage of Non-Functional water point is randomly distributed across the country. And the **statistic = 0.44897 \< 1.** For the **c statistic,** if it is Small c value (\<1) : Clustered, observations tend to be similar.
